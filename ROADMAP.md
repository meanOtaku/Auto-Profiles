...
| M8 | Candidate enrollment | Implementation drafted; verification deferred | [M8 report](docs/reports/M8.md): candidate lifecycle, qualification (consistency, temporal-variability, duplicate, near-frontal checks), and atomic/auditable/idempotent promotion were added on 2026-08-01. Manual review remains default; the later owner-approved automatic mode reuses the same promoter behind explicit consent acknowledgement plus liveness, encryption, full-pipeline, retention, and API-security gates. Automated verification remains deferred by owner instruction. |
...
| M17 | Native Windows compatibility | Implementation and Windows safe-path verification complete; hardware runtime deferred | [M17 report](docs/reports/M17.md): native PowerShell launch/provision paths, DirectShow selection, pycaw/WMI settings, protected-DACL storage, and Windows configs. Windows workflow run 30884122482 passed the Python 3.11/3.12 packaging/import/hardware-disabled CLI matrix without tests; hardware checks remain pending. |
